...
using WebServiceSecured.Models;
using WebServiceSecured.Models.Account;
using WebServiceSecured.Models.Domain;
using WebServiceSecured.Providers;
using WebServiceSecured.Results;
...
namespace WebServiceSecured.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Identity()
        {
            ...
            return Ok(identityModel);
        }

        // POST api/Account/Register
        [AllowAnonymous]
        [Route("Register")]
        public async Task<IHttpActionResult> RegisterUser(RegisterModel model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (domain.Equals("capitec.co.za"))
            {
                var x = username[0];
                switch (x)
                {
            ...
            IdentityUser y = await UserManager.FindByEmailAsync(user.Email);
            ...
            var claimResult = await UserManager.AddClaimAsync(user.Id, new Claim(ClaimTypes.Role, claim));
